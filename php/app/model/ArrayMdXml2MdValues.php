...
class ArrayMdXml2MdValues extends \BaseModel
{
    ...
    private function setLangsMd($recno, $iso_lang) {
        ...
        case 'DC':
            break;
        case 'FC':
            if(isset($this->arrayXml['FC_FeatureCatalogue'][$idx]['locale']) && is_array($this->arrayXml['FC_FeatureCatalogue'][$idx]['locale'])) {
                foreach ($this->arrayXml['FC_FeatureCatalogue'][$idx]['locale'] as $value) {
                    $l = $value['PT_Locale']['00']['languageCode']['00']['LanguageCode']['00']['@'];
                    switch ($l) {
                        case 'en':
                            $l = 'eng';
                ...
                unset($this->arrayXml['FC_FeatureCatalogue'][$idx]['locale']);
            } elseif (isset($this->arrayXml['FC_FeatureCatalogue'][$idx]['name']['00']) && is_array($this->arrayXml['FC_FeatureCatalogue'][$idx]['name']['00'])) {
                foreach ($this->arrayXml['FC_FeatureCatalogue'][$idx]['name']['00'] as $key => $value) {
                    if (substr($key,0,1) == '@') {
                        $l = substr($key,1);
                        switch ($l) {
                            case '':
    ...
    private function setMd()
    {
        ...
        if (array_key_exists('FC_FeatureCatalogue', $this->arrayXml)) {
            foreach ($this->arrayXml['FC_FeatureCatalogue'] as $idx=>$md) {
                ...
                $this->md[$recno]['uuid'] = 
                    isset($this->arrayXml['FC_FeatureCatalogue'][$idx]['id']['00']['@']) && $this->arrayXml['FC_FeatureCatalogue'][$idx]['id']['00']['@'] != ''
                    ? $this->arrayXml['FC_FeatureCatalogue'][$idx]['id']['00']['@']
                    : '';
                if ($this->md[$recno]['uuid'] != '') {
