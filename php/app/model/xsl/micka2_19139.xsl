			...
			<xsl:for-each select="identificationInfo/*/descriptiveKeywords/*">
				<gmd:descriptiveKeywords>
				  	<gmd:MD_Keywords>				  		
                        ...
                        <!-- xsl:for-each select="keyword[not(@i = preceding-sibling::keyword[1]/@i)]">
                            <xsl:variable name="ii" select="@i" />
                            <xsl:variable name="cnt" select="count(../keyword[@i = $ii])"/>                            
                            <xsl:choose>
   		  						<xsl:when test="$cnt>1">
  			  					 	<gmd:keyword xsi:type="gmd:PT_FreeText_PropertyType">
                   						<xsl:choose>
                     			           	<xsl:when test="../keyword[@i=$ii and @lang='uri']">
									  			<gmx:Anchor xlink:href="{../keyword[@i=$ii and @lang='uri']}">
									  				<xsl:value-of select="../keyword[@i=$ii and @lang=$mdLang]"/>
									  			</gmx:Anchor>
									  		</xsl:when>	
                                            <xsl:when test="../keyword[@i=$ii and @lang=$mdLang]">
                                                <gco:CharacterString><xsl:value-of select="../keyword[@i=$ii and @lang=$mdLang]"/></gco:CharacterString>
  			  					 		    </xsl:when>
                                        </xsl:choose>     
                                        <gmd:PT_FreeText>
	  				  				  		<xsl:for-each select="../keyword[@i=$ii and @lang!=$mdLang and @lang!='uri']">
	 				   					  		<gmd:textGroup>
							...
							<gfc:FC_FeatureAttribute>
								...
								<xsl:if test="valueType">
									<gfc:valueType>
										<gco:TypeName>
											<gco:aName>
												<gco:CharacterString><xsl:value-of select="valueType"/></gco:CharacterString>
											</gco:aName>
										</gco:TypeName>	
									</gfc:valueType>
								</xsl:if>
								<xsl:for-each select="listedValue">
									<gfc:listedValue>
										<gfc:FC_ListedValue>
											<xsl:call-template name="ftxt">
												<xsl:with-param name="s" select="valueLabel"/>                      
											...
											<xsl:if test="valueCode">
												<gfc:code>
													<gco:CharacterString><xsl:value-of select="valueCode"/></gco:CharacterString>
												</gfc:code>
											</xsl:if>
											<xsl:if test="valueDefinition">
												<xsl:call-template name="ftxt">
													<xsl:with-param name="s" select="valueDefinition"/>                      
													<xsl:with-param name="name" select="'definition'"/>                      
													<xsl:with-param name="lang" select="$mdLang"/>
													<xsl:with-param name="ns" select="'gfc'"/>                     
												</xsl:call-template>
