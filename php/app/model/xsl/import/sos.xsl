...
<xsl:stylesheet version="1.0" 
  ...
  <results>
    ...
    <serviceType>
      <gco:LocalName>SOS</gco:LocalName>
              ...
              <CI_OnlineResource>
              	...
              	<xsl:choose>
              		<xsl:when test="substring-after(translate(../../../@name,$upper,$lower),'get')!=''">
              			<protocol>OGC:SOS-<xsl:value-of select="//@version"/>-http-<xsl:value-of select="translate(substring-after(name(),':'),$upper,$lower)" />-<xsl:value-of select="substring(translate(../../../@name,$upper,$lower),4)"/></protocol>
              		</xsl:when>
              		<xsl:otherwise>
               			<protocol>OGC:SOS-<xsl:value-of select="//@version"/>-http-<xsl:value-of select="translate(substring-after(name(),':'),$upper,$lower)" />-<xsl:value-of select="translate(../../../@name,$upper,$lower)"/></protocol>             		
              		</xsl:otherwise>
              	</xsl:choose>		
              </CI_OnlineResource>
            </connectPoint>
    ...
    <MD_Distribution>
      ...
      <MD_DigitalTransferOptions>
        <onLine>
        <CI_OnlineResource>
          <linkage><xsl:value-of select="*/ows:OperationsMetadata/ows:Operation[@name='GetCapabilities']/ows:DCP/ows:HTTP/ows:Get/@xlink:href"/></linkage>
          <protocol>OGC:SOS-<xsl:value-of select="*/@version"/>-http-get-capabilities</protocol>
