...
<xsl:template match="rec/gmd:MD_Metadata" xmlns="http://www.openarchives.org/OAI/2.0/" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco">
 <record>
   <header>
   ...
   <metadata>
    <oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/2.0/oai_dc/ http://www.openarchives.org/OAI/2.0/oai_dc.xsd"> 
      ...
      <dc:identifier>urn:uuid:<xsl:value-of select="gmd:fileIdentifier"/></dc:identifier>        
      ...
      <xsl:for-each select="gmd:identificationInfo/*/gmd:citation/*/gmd:title/gco:CharacterString">
      ...
      <xsl:for-each select="gmd:identificationInfo/*/gmd:pointOfContact">
	       <xsl:choose>
          	...
          	<xsl:when test="gmd:role/gmd:CI_RoleCode_CodeList='publisher'">
            	<dc:publisher> <xsl:value-of select="organisationName/gco:CharacterString"/> </dc:publisher>
          	</xsl:when>
         	  <xsl:when test="gmd:role/gmd:CI_RoleCode_CodeList='author'">
            	<dc:contributor> <xsl:value-of select="organisationName/gco:CharacterString"/> </dc:contributor>
          	</xsl:when>
      ...
      <xsl:for-each select="gmd:identificationInfo/*/gmd:citation/*/gmd:date">
       		<dc:date><xsl:value-of select="*/gmd:date"/></dc:date>
	  </xsl:for-each> 
      ...
      <dc:coverage>northlimit=<xsl:value-of select="gmd:northBoundLatitude"/>; southlimit=<xsl:value-of select="gmd:southBoundLatitude"/>; eastlimit=<xsl:value-of select="gmd:eastBoundLongitude"/>; westlimit=<xsl:value-of select="gmd:westBoundLongitude"/>;</dc:coverage>
      </xsl:for-each>
