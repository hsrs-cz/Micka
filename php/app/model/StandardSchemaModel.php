...
class StandardSchemaModel
{
    ...
    private $sqlWhereTree = '
        WHERE standard_schema.[md_standard]=%i
            AND standard_schema.[md_id]=%i
            AND standard_schema.[md_left]>=%i
            AND standard_schema.[md_right]<=%i
    ';
    ...
    public function getMdStandard($md_standard) {
        return $this->db->query('SELECT [*] FROM standard WHERE [md_standard]=%i', $md_standard)->fetch();
    }
    ...
    public function getNodeFull($node) {
        return $this->db->query($this->sqlSelectTree.' WHERE standard_schema.[md_standard]=? AND [md_id]=%i', $this->md_standard, $node)->fetch();
    }
