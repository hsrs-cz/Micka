...
class MdXml2Array
{
    ...
    private function writeNode($path, $node, $idx)
    {
        ...
        if(($node->nodeType==XML_ELEMENT_NODE)&&($node->hasAttribute('xlink:href'))){
            $s .= $path."['".$node->nodeName."']['$idx']['href']['00']['@']='".$node->getAttribute('xlink:href')."';\n";
        }
        if(($node->nodeType==XML_ELEMENT_NODE)&&($node->hasAttribute('xlink:title'))){
            $s .= $path."['".$node->nodeName."']['$idx']['title']['00']['@']='".$node->getAttribute('xlink:title')."';\n";
        }
        if(($node->nodeType==XML_ELEMENT_NODE)&&($node->hasAttribute('uuidref'))){
            $s .= $path."['".$node->nodeName."']['$idx']['uuidref']['00']['@']='".$node->getAttribute('uuidref')."';\n";    
        }
        ...
        if($node->nodeType==XML_ELEMENT_NODE && $node->hasAttribute('id') && $node->nodeName=='featureCatalogue'){
            $s .= $path."['".$node->nodeName."']['$idx']['id']['00']['@']='".trim($node->getAttribute('id'))."';\n";
        }

        if($node->nodeType!=XML_TEXT_NODE && $node->nodeType!=XML_COMMENT_NODE){
            //atributy TODO - zobecnit
            if($node->hasAttribute('uom')){
                $s .= $path."['".$node->nodeName."']['$idx']['uom']['00']['@']='".$node->getAttribute('uom')."';\n";
            }
        ...
        if(($node->nodeType==XML_ELEMENT_NODE)&&($node->hasAttribute('codeListValue'))){
            $s .= $path."['".$node->nodeName."']['00']['@']='".addslashes(trim($node->getAttribute('codeListValue')))."'; \n";
        } 
        // locales - taky nejde dal
            ...
            foreach($node->childNodes as $ch){  
                if($ch->nodeType == XML_ELEMENT_NODE && $ch->hasAttribute('locale')){
                    ...
                    $tmploc = isset($locales[$locale]) ? $locales[$locale] : '';
                    $path .= "['@".$tmploc."']";
    ...
    function xml2array($xml, $template, $params=array())
    {
        ...
        $data = str_replace(
            array("['language']['00']['gco:CharacterString']", "['MD_Identifier']", "ns1:" ) , 
            array("['language']['00']['LanguageCode']", "['RS_Identifier']", "gco:" ), 
            $data); //kvuli portalu INSPIRE

        // quick and dirty patch for distance
        $data = str_replace(
            Array("['gco:Distance']['00']['uom']", "['gco:Distance']"),
            Array("['uom']['00']['uomName']", "['value']"), 
            $data
        );
        // DEBUG
        //echo "<pre>$data</pre>"; exit;  
        ...
        $data = str_replace("['language']['00']['gco:CharacterString']" , "['language']['00']['LanguageCode']", $data); //kvuli portalu INSPIRE
        ...
        "['PT_FreeText']['00']", "[]['00']", "'DCPList'", "['gts:TM_PeriodDuration']['00']", "['Polygon']['00']['exterior']['00']['LinearRing']['00']['posList']['00']",
                "['Polygon']['00']['exterior']['00']['Ring']['00']['curveMember']['00']['LineString']['00']['coordinates']['00']",
                "'gmx:MimeFileType'");
        $data= str_replace($elim , "", $data);
        $data= str_replace("['serviceType']['00']" , "['serviceType']['00']['LocalName']['00']", $data);  
        $data= str_replace(
            array(	
            "['begin']['00']['TimeInstant']['00']['timePosition']['00']", 
            "['end']['00']['TimeInstant']['00']['timePosition']['00']",
            "['MD_Identifier']",
            "'false'", "'true'", // predpokladam, ze jde o boolean
            "MI_Metadata",
            "['Anchor']['00']['href']['00']['@']",
            "['Anchor']['00']",
            "['graphicOverview']['00']['href']", // TODO pro vice
            "['graphicOverview']['00']['title']"
            ),
            array(
            "['beginPosition']['00']", 
            "['endPosition']['00']",
            ...
            "['@uri']",
            "",
            "['graphicOverview']['00']['MD_BrowseGraphic']['00']['fileName']",
            "['graphicOverview']['00']['MD_BrowseGraphic']['00']['fileDescription']"
            ), 
            $data
        );    
        ...
        //*** pro DC
        $data = str_replace(
            Array("csw:Record","dc:","dct:abstract", "[]['00']"),
        ...
        */
        ...
        if(isset($md['MD_Metadata'])) for($i=0; $i<count($md['MD_Metadata']); $i++){
            ...
            if(isset($md['MD_Metadata'][$i]['dateStamp']['00']['@']) && strpos($md['MD_Metadata'][$i]['dateStamp']['00']['@'], 'T')){
                ...
                $md['MD_Metadata'][$i]['dateStamp']['00']['@'] = $pom[0]; 
            }

            // zpracovani polygonu
            for($j=0; $j< isset($md['MD_Metadata'][$i]['identificationInfo']['00']['MD_DataIdentification']['00']['extent']['00']['EX_Extent']['00']['geographicElement']) ? count($md['MD_Metadata'][$i]['identificationInfo']['00']['MD_DataIdentification']['00']['extent']['00']['EX_Extent']['00']['geographicElement']) : 0; $j++){
                if(isset($md['MD_Metadata'][$i]['identificationInfo'][0]['MD_DataIdentification']['00']['extent']['00']['EX_Extent']['00']['geographicElement'][$j]['EX_BoundingPolygon']['00']['polygon']['00']['@'])){
                    $geom = explode(" ",$md['MD_Metadata'][$i]['identificationInfo']['00']['MD_DataIdentification']['00']['extent']['00']['EX_Extent']['00']['geographicElement'][$j]['EX_BoundingPolygon']['00']['polygon']['00']['@']);
                    $result="";
                    for($k=0; $k<count($geom); $k=$k+2){
                        if($result) $result .=",";
                        $result .= $geom[$k]." ".$geom[$k+1];
                    }
                    $md['MD_Metadata'][$i]['identificationInfo']['00']['MD_DataIdentification']['00']['extent']['00']['EX_Extent']['00']['geographicElement'][$j]['EX_BoundingPolygon']['00']['polygon']['00']['@'] = "MULTIPOLYGON(((".$result.")))";
                }
            }
            // doplnění překladu INSPIRE
            // --- multiligvalni klic. slova
            $lang = isset($md['MD_Metadata'][$i]["language"]['00']["LanguageCode"]['00']['@'])
                    ? $md['MD_Metadata'][$i]["language"]['00']["LanguageCode"]['00']['@']
                    : '';
            if(!$lang) $lang='eng';
    ...
    function multiKeywords(&$keywords, $lang)
    {
        ...
        for($i=0; $i<count($keywords); $i++){
            for($j=0; $j< isset($keywords[$i]['MD_Keywords']['00']['keyword']) ? count($keywords[$i]['MD_Keywords']['00']['keyword']) : 0; $j++){
                if(isset($keywords[$i]['MD_Keywords']['00']['keyword'][$j]['gmx:Anchor'])){
                    $keywords[$i]['MD_Keywords']['00']['keyword'][$j]['@uri'] = $keywords[$i]['MD_Keywords']['00']['keyword'][$j]['gmx:Anchor']['00']['href']['00']['@'];
                    $keywords[$i]['MD_Keywords']['00']['keyword'][$j]['@'] = $keywords[$i]['MD_Keywords']['00']['keyword'][$j]['gmx:Anchor']['00']['@'];
                    unset($keywords[$i]['MD_Keywords']['00']['keyword'][$j]['gmx:Anchor']);
                }
            }
            $thesaurusName = 
                    isset($keywords[$i]['MD_Keywords']['00']['thesaurusName']['00']['CI_Citation']['00']['title']['00']['@'])
                    ? $keywords[$i]['MD_Keywords']['00']['thesaurusName']['00']['CI_Citation']['00']['title']['00']['@']
                    : '';
            // --- jen INSPIRE zatím 
            if(strpos($thesaurusName, 'INSPIRE')!==false){
                // --- cyklus pres klíčová slova
                for($j=0; $j<count($keywords[$i]['MD_Keywords']['00']['keyword']); $j++){
                    $kwpair = $this->translateKeyword($keywords[$i]['MD_Keywords']['00']['keyword'][$j]['@'], $lang);
                    if($kwpair){
                        $keywords[$i]['MD_Keywords']['00']['keyword'][$j]['@uri'] = $kwpair[0];
                        $keywords[$i]['MD_Keywords']['00']['keyword'][$j]['@eng'] = $kwpair[1];
                    }
                }
            }
        }
    ...
    function getArrayMdFromXml($xmlString, $langs, $lang_main, $params=false,$updateType="", $md_rec="", $fc="")
    {
        ...
        //echo "<pre>"; var_dump($md); exit;
        $lang=$md['MD_Metadata']['00']["language"]['00']["LanguageCode"]['00']['@'];
        if($lang=='fra') $lang="fre"; // kvuli 1GE
        if (!$lang && $lang_main != '') $lang = $lang_main;
        $md['MD_Metadata']['00']["language"]['00']["LanguageCode"]['00']['@'] = $lang;
        ...
        if($fc != '') {
            ...
            $uuid = $md['MD_Metadata']['00']['fileIdentifier']['00']['@'];
            ...
            if($esri){
                ...
                $md['MD_Metadata']['00']['identificationInfo']['00'][$pom]['00']['graphicOverview']['00']['MD_BrowseGraphic']['00']['fileName']['00'] = "http://".$_SERVER['SERVER_NAME'].dirname($_SERVER['PHP_SELF'])."/graphics/$uuid.png";        
                $md['MD_Metadata']['00']['identificationInfo']['00'][$pom]['00']['graphicOverview']['00']['MD_BrowseGraphic']['00']['fileDescription']['00'] = "náhled";        
                }
            }
        }
        if ($md_rec != '') {
