...
<!-- IDENTIFIKACE -->
 ...
 <xsl:choose>
	...
	<!--xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="value" select="gmd:identificationInfo/*/gmd:resourceConstraints/*/gmd:useLimitation[1]"/>
	    <xsl:with-param name="class" select="'mandatory'"/> 
	    <xsl:with-param name="type" select="'textarea'"/>
	...
	<xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="value" select="gmd:identificationInfo/*/gmd:resourceConstraints/*/gmd:otherConstraints[1]"/>
	    <xsl:with-param name="class" select="'mandatory'"/> 
	    <xsl:with-param name="type" select="'textarea'"/>
...
<xsl:if test="not($serv)">
    ...
    <div class="row">
        ...
        <span class="col-xs-12 col-md-8">
            <select class="sel2" multiple="multiple" data-allow-clear="true" data-tags="true" data-ajax--url="../../registry_client/?lang=cs&amp;uri=https://inspire.ec.europa.eu/metadata-codelist/LimitationsOnPublicAccess">
                <xsl:for-each select="gmd:identificationInfo/*/gmd:resourceConstraints/*/gmd:otherConstraints">
                    <option value="{*/@xlink:href}" selected="'selected'">
                        <xsl:call-template name="multiText">
...
<xsl:for-each select="gmd:identificationInfo/*/gmd:resourceMaintenance|/.">
	<xsl:if test="string-length(*/gmd:maintenanceAndUpdateFrequency/*/@codeListValue)>0 or(string-length(*/gmd:maintenanceAndUpdateFrequency/*/@codeListValue)=0 and position()=last())">
		...
		<fieldset style="clear:both" class="cl" id="maintenance_{$row}_">
			...
			<span class="duplicate"><i class="fa fa-clone"></i></span> 	
