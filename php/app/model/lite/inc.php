...
if($_REQUEST['action']){
	...
	echo $cswClient->processTemplate($input,dirname(__FILE__).'/profiles/kote-micka/form2iso.xsl', $params);
}
// prvni nacteni
else {
	$params = array('recno'=>1,'select_profil'=>5);
	$s = file_get_contents("resources/priklad_inspire.xml");
	echo $cswClient->processTemplate($s, dirname(__FILE__).'/profiles/kote/iso2form.xsl',$params);
}	 