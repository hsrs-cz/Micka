...
<xsl:stylesheet version="1.0" 
  ...
  xmlns:gco="http://www.isotc211.org/2005/gco" >
	...
	<xsl:variable name="CLIENT" select="'../ak=detail&amp;uuid='"/>
	...
	<xsl:template match="/">
		...
		<tr>
			...
			<th>Dataset</th>
            <th>UUID</th>
			<th>Prohlížecí služba + WMS</th>
			<th>Stahovací služba + WFS</th>
		</tr>
		...
		<xsl:for-each select="*/rec">
			...
			<tr>
				...
				<!-- sluzby -->
				<xsl:variable name="vazby" select="php:function('getMetadata', concat('(uuidRef=',*/gmd:fileIdentifier/*,' or operateson like *', */gmd:fileIdentifier/*, '*)  and (serviceType=download or serviceType=view or serviceType=WMS or serviceType=WFS)'), 'extended')"/>
				<td>
                    <!-- view -->
					<xsl:for-each select="$vazby//rec[*/gmd:identificationInfo/*/srv:serviceType='view' or */gmd:identificationInfo/*/srv:serviceType='WMS']">
				...
				<td>
                    <!-- download -->
					<xsl:for-each select="$vazby//gmd:MD_Metadata[gmd:identificationInfo/*/srv:serviceType='download' or gmd:identificationInfo/*/srv:serviceType='WFS']">
						<xsl:variable name="mdlang1" select="gmd:language/*/@codeListValue"/>
		...
		<div class="dole">
			<a href="?request=GetRecords&amp;format=text/html&amp;language=cze&amp;query=type%3Ddataset%20or%20type%3Dseries%20and%20ForInspire%3D1&amp;sortby=&amp;template=report-cenia-csv&amp;maxrecords=200">Stáhnout jako CSV</a>
		</div>
		</body>
		</html>
	</xsl:template>
